...
from server.repository import Song, SongIndexer, Base, SongVersion
...
def test_index_songs_added(db_session, mock_parse_text_file):
    ...
    added_versions = db_session.query(SongVersion).all()

    assert len(added_songs) == len(mock_files)
    assert len(added_versions) == len(mock_files)
    ...
    assert added_songs[1].folder == '/path/to/folder2'
...
def test_index_songs_edited(db_session, mock_parse_text_file):
    ...
    existing_version = SongVersion(folder='/path/to/folder2', txt_path='/path/to/song2.txt', is_duet=True, is_rap=False)
    ...
    updated_version = added_versions[0]
    assert updated_version.folder == '/path/to/folder2'
    assert updated_version.is_rap
    assert updated_version.is_duet
    ...
    assert updated_song.folder == '/path/to/folder2'
...
def test_edit_new_version(db_session, mock_parse_text_file):
    # Mock data for testing
    mock_files = [
    ...
    mock_parse_text_file.return_value = {
        ...
        'Mp3Path': key,
        'ModifyDate': 1234567890,
        'TxtPath': '/path/to/song4.txt',
        'Folder': '/path/to/folder2'
    }

    # already added the second song
    existing_song = Song(mp3_path=key, title='Original Title', is_duet=True, is_rap=True)
    existing_version = SongVersion(folder='/path/to/folder2', txt_path='/path/to/song2.txt',
                                   is_duet=True, is_rap=True)
    db_session.add(existing_song)
    db_session.add(existing_version)
    db_session.commit()
    ...
    new_version = db_session.query(SongVersion).filter_by(txt_path='/path/to/song4.txt').first()
    assert new_version.folder == '/path/to/folder2'
    assert not new_version.is_duet
    assert not new_version.is_rap
